...
public class Issuer {
    ...
    public final Message round2(final Message msg) {
      return round2(msg, null);
    }
    
    public final Message round2(final Message msg, final IdemixSmartcardHelper smartcardManager) {
        ...
        if(spec.getCredentialStructure().isOnSmartcard()) {
          BigInteger zx = (BigInteger) sValues.get(IssuanceSpec.SMARTCARD_SECRET).getValue();
          BigInteger R0 = capR[IssuanceSpec.SMARTCARD_SECRET_INDEX];
          capUHat = capUHat.multiply(R0.modPow(zx, n)).mod(n);
        }

        final HashMap<String, BigInteger> capCHat = getCHat(negC, sValues);

        BigInteger cHat = Utils.computeFSChallenge(sp, spec.getContext(),
                capU, attStructs, values, null, null, capUHat, capCHat,
                null, null, nonce1);
        if(spec.getCredentialStructure().isOnSmartcard()) {
          log.log(Level.INFO, "on smartcard: hashing twice...");
          byte[] preimage = cHat.toByteArray();
          cHat = smartcardManager.computeChallenge(preimage, msg.getProof().getSmartcardNonce());
        }
        ...
        if (updateSpecLocation != null) {
            ...
            response = new Message(issuanceProtocolValues, p2, updateLocation, 2);
        } else {
            response = new Message(issuanceProtocolValues, p2, 2);
        }
        return response;
    }
