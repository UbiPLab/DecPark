...
public class Utils {
     ...
     * 
     * This implementation uses the algorithm proposed by Ronald Cramer
    ...
    public static BigInteger hashOf(final int l_H, final BigInteger[] array) {
        ...
        // System.err.println("hash: " + l_H + " : " + Arrays.toString(array));
    ...
    public static BigInteger multiExpMul(
        ...
        if (Constants.USE_MULTI_CORE_EXP) { // use multi-core exponentiation.
            ...
            return mcmb.multiBaseExp(exponentiations, modulus);
             ...
             Constants.printCalculationLog("#EXP start# ");
             BigInteger res = MultiCoreMultiBase.compute(exponentiations, modulus);
             Constants.printCalculationLog("#EXP modulus# " + modulus);
             Constants.printCalculationLog("#EXP result# " + res);
             return res;
        }
    }
    ...
    public static final BigInteger computeFSChallenge(
        ...
        log.log(logLevel, "Length: " + len);
        log.log(logLevel, "context: " + Utils.logBigInt(context));
        log.log(logLevel, "capU: " + Utils.logBigInt(capU));
        log.log(logLevel, "U1: " + Utils.logBigInt(capUTilde));
        ...
        if (nym != null) {
            arr[i++] = nym;
            log.log(logLevel, Utils.logBigInt(nym));
        }
        if (domNym != null) {
            arr[i++] = domNym;
            log.log(logLevel, Utils.logBigInt(domNym));
        }
        j = 0;
        arr[i++] = capUTilde;
        for (AttributeStructure attStruct : attStructs) {
        ...
        if (nym != null) {
            arr[i++] = nymTilde;
            log.log(logLevel, Utils.logBigInt(nymTilde));
        }
        if (domNym != null) {
            arr[i++] = domNymTilde;
            log.log(logLevel, Utils.logBigInt(domNymTilde));
        }
        arr[i++] = n1;
    ...
    public static BigInteger computeChallenge(final SystemParameters sp,
        ...
        log.log(logLevel, "computing challenge[Hat]: len = " + len);
        log.log(logLevel, " context = " + Utils.logBigInt(context));
        ...
        log.log(logLevel, "array to be hashed:" + Utils.logArray(arr));
        log.log(logLevel,
